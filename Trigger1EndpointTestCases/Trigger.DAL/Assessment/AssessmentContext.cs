...
namespace Trigger.DAL.Assessment
{
    public class AssessmentContext
    {
        ...
        public virtual int AddAssessmentMainV1(EmpAssessmentModel empAssessmentModel)
        {
            int result = 0;
            try
            {
        ...
        /// <returns></returns>
        public async Task<int> AddAssessmentMainV2(EmpAssessmentModel empAssessmentModel)
        {
            int result = 0;
            ...
            try
            {
                ...
                if (assessmentID > 0)
                {
                    result = InsertAssessmentDetailsV2(assessmentID, uploadDocuments, empAssessmentModel);
                    if (result > 0)
                    {
                        result = UpdateScoreRank(new EmpAssessmentScore { empId = empAssessmentModel.empId, assessmentId = assessmentID });
