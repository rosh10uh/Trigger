...
BEGIN
	...
	SELECT TOP 1  a.id as assessmentId, e.CompanyId, e.Empid, dbo.[GetEmpNameById](null,e.firstname,e.middlename,e.lastname) AS EmpName,
		   a.score AS empScore, d.scorerank AS empScoreRank, 
		   d.GeneralScoreRank, d.ScoreRemarks, d.ManagerAction, d.ScoreSummary, 
	...
	WHERE e.companyid=@CompanyId and  a.empid=@EmpId
	ORDER BY a.id DESC
...
(
	...
	INNER JOIN 
	(
			SELECT TOP 1 l.score, l.scoreRank, l.empName, noofcount,noofcountlyr, l.empid, dbo.GETSCORERANK(vg.avgscore) AS curravgscorerank, vg.avgscore AS curravgscore,
			CAST(ROUND(lyrScore,0) AS DECIMAL(10,0)) AS lyrScore, l.scoreremarks, l.ManagerAction, l.ScoreSummary, l.GeneralScoreRank, l.RatingDate
			FROM vw_EmpLastAssessmentDet l 
			LEFT JOIN 
			(
...
SET @UserRole = Dbo.GETRole(@UserId)
    -- Insert statements for procedure here
...
ELSE 
	IF @UserRole IN (2,4)
	BEGIN
		SELECT uw.UserId, wm.Id AS WidgetId, wm.WidgetName AS WidgetName, wm.WidgetactualName, CAST(uw.IsActive AS BIT) AS IsActive,
			uw.SequenceNumber, uw.TileSequence, uw.Position
		FROM [dbo].[UserWiseWidgetDetails] uw
		LEFT JOIN [dbo].[WidgetMaster] wm ON wm.id = uw.WidgetId
		WHERE wm.RoleId IN (4, 0) AND uw.UserID = @UserId AND wm.widgettypeid = @WidgetType
		ORDER BY wm.Id
	END
	ELSE
	BEGIN
		...
		FROM [dbo].[UserWiseWidgetDetails] uw
		LEFT JOIN [dbo].[WidgetMaster] wm ON wm.id = uw.WidgetId
		WHERE wm.RoleId IN (@UserRole, 0) AND uw.UserID = @UserId AND wm.widgettypeid = @WidgetType
		ORDER BY wm.Id
	END	
END
----------
...
BEGIN
	...
	(
		...
		GROUP BY e.empid, e.employeeId,e.companyid, e.firstname, e.middlename, e.lastname, e.email, e.jobtitle,  e.joiningdate,
				...
				ISNULL(vg.AvgScoreRank, ''),vg.AvgScore, IsMailSent, e.currentsalary,empImgPath
	)er
...
BEGIN
	SELECT empid, employeeId, companyid, firstname, middlename, lastname, email, jobtitle, joiningdate,
		   ...
		   lastAssessedDate, AvgScoreRank, AvgScore, lastScoreRank, RatingCompleted, ord
...
BEGIN  	
	...
	SELECT @TotalRowCount AS totalrowcount,empid, employeeId, companyid, firstname, middlename, lastname, email, jobtitle, joiningdate,
		   ...
		   lastAssessedDate, AvgScoreRank, AvgScore,lastScoreRank, RatingCompleted
	FROM #tempEmployeeResult
	WHERE (1=(CASE WHEN @SearchString = 'all records' THEN 1 ELSE 0 END) 
	OR employeeId like '%'+ @SearchString + '%'
	OR lastname like '%'+ @SearchString + '%'
...
BEGIN
		...
		GROUP BY e.empid, e.employeeId,e.companyid, e.firstname, ISNULL(e.middlename, ''), e.lastname, e.email, e.jobtitle,  e.joiningdate,
				...
				ISNULL(vg.AvgScoreRank, ''),vg.AvgScore, ISNULL(e.IsMailSent,0), e.currentsalary
...
BEGIN
	SELECT e.empid, e.employeeId, e.companyid, cd.companyName, e.firstname, ISNULL(e.middlename, '') AS middlename, e.lastname, 
		   ...
		   '' AS lastAssessedDate, 0 as AvgScore, '' AS AvgScoreRank, '' AS lastScoreRank, '' AS RatingCompleted,
		   0 AS ord, e.IsMailSent, e.currentsalary
	FROM employeedetails e
	LEFT JOIN EmployeeDetails m ON m.bactive = 1 AND m.empId = e.managerId
	LEFT JOIN department d ON d.departmentid = e.departmentid AND d.bactive = 1
	LEFT JOIN companydetails cd ON cd.compid = e.companyId AND cd.bactive = 1
...
AS
BEGIN
IF NOT EXISTS(SELECT * FROM employee_assessment WHERE empid = @empid AND  CONVERT(DATE,assessmentdate,110) = CONVERT(DATE,@assessmentdate,110))
	BEGIN
	-- Insert statements for procedure here
			INSERT INTO employee_assessment
			(
