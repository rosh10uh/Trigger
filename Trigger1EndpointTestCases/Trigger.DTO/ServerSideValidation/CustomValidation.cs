...
namespace Trigger.DTO.ServerSideValidation
{
    ...
    {
        private string _countryIdFieldName { get; set; }
        public RegionValidationAttribute(string countryIdFieldName)
        {
            _countryIdFieldName = countryIdFieldName;
        }
        protected override ValidationResult IsValid(object value, ValidationContext validationContext)
        {
            int region = (int)value;
            int contryId = (int)validationContext.ObjectType.GetProperty(_countryIdFieldName).GetValue(validationContext.ObjectInstance, null);
