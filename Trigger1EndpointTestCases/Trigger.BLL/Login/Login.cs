...
namespace Trigger.BLL.Login
{
    public class Login
    {
        ...
        public Login(IConnectionContext connectionContext, ILogger<Login> logger, IActionPermission actionPermission, 
            Notification.Notification notification, IOptions<AppSettings> appSettings)
        {
        ...
        #region Login
        public virtual JsonData InvokeLogin(UserLoginModel userLoginModel, string apiVersion)
        {
            try
            {
        ...
        private void SendNotifications(EmployeeModel employee, int result)
        {
            if (result > 0)
            {
                  _notification.InvokeNotificationAsync(employee).Wait();
            }
        }

        private UserDataModel SetUserData(UserDataModel userData, int empId, string apiVersion)
        {
            string storageAccountURL = _appSettings.StorageAccountURL;
            string employeeBlob = storageAccountURL + Messages.slash + _blobContainerEmployee + Messages.slash;
            string companyBlob = storageAccountURL + Messages.slash + _blobContainerCompany + Messages.slash;
...
>>>>>>> 2157f8a3dbc2f1e97f86e394aca030ca64e97686
        ...
        /// <returns></returns>
        public virtual JsonData InvokeDeleteDeviceInfo(string deviceId)
        {
            try
            {
