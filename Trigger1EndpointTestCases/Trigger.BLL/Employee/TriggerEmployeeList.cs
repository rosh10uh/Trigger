...
{
    public class TriggerEmployeeList
    {
        ...
        public TriggerEmployeeList(IConnectionContext connectionContext, IClaims Claims, IActionPermission actionPermission, ILogger<DashboardEmployeeList> logger, Employee employee)
        {
        ...
        public async Task<CustomJsonData> GetTriggerEmpListV2_1(int companyId, int managerId)
        {
            try
            {
                ...
                {
                    ActionList allPermission = await Task.FromResult(_actionPermission.GetPermissionsV2_1(managerId == 0 ? Convert.ToInt32(_iClaims["EmpId"].Value) : managerId).FirstOrDefault(x => (x.ActionId == Enums.Actions.TriggerEmployee.GetHashCode())));
