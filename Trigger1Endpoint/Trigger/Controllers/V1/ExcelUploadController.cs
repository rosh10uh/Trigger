...
namespace Trigger.Controllers.V1
{
    ...
    public class ExcelUploadController : ControllerBase
    {
        ...
        [DynamicConnection]
        public async Task<ActionResult<CustomJsonData>> Post([FromRoute]string companyId, [FromBody] List<CsvEmployeesModel> employees)
        {

            var countRecord = new CountRecordModel()
            {
                companyId = companyId,
                lstNewCsvUpload = employees
            };
        ...
        [DynamicConnection]
        public async Task<ActionResult<CustomJsonData>> PostExcel([FromRoute]string companyId, [FromBody] List<CsvEmployeesModel> employees)
        {
            var countRecord = new CountRecordModel()
            {
                companyId = companyId,
