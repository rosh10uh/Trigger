...
CREATE OR ALTER FUNCTION [dbo].[GetLastAssessmentDateByManager] 
(
	@ManagerId INT 
)
RETURNS  TABLE
AS
RETURN
SELECT e.empid ,MAX(assessmentdate) as assessmentdate
FROM employee_assessment  e
INNER JOIN UserLogin u on u.userid=e.assessmentby
WHERE u.empid=@ManagerId
GROUP BY e.empid, u.userid 
