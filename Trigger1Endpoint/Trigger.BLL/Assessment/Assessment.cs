...
namespace Trigger.BLL.Assessment
{
    public class Assessment
    {
        ...
        /// <returns></returns>
        public async Task<JsonData> GetAssessmentScore(int CompanyId, int EmpId)
        {
        ...
        /// <summary>
        /// Use to get assessment score from assessment id and inserted pk
        /// </summary>
        /// <param name="CompanyId"></param>
        /// <param name="EmpId"></param>
        /// <param name="inserted"></param>
        /// <returns></returns>
        private async Task<JsonData> GetAssessmentScoreById(int assessmentId, int inserted)
        {
            try
            {
                AssessmentScoreModel assessmentScoreModel = new AssessmentScoreModel() { AssessmentId= assessmentId };
                var result = await Task.FromResult(_connectionContext.TriggerContext.AssessmentScoreRepository.GetAssessmentScore(assessmentScoreModel));
                if (inserted > 0)
                {
                    return JsonSettings.UserDataWithStatusMessage(result, Convert.ToInt32(Enums.StatusCodes.status_200), Messages.ok);
        ...
        /// <returns></returns>
        public async Task<JsonData> AddAssessmentAsyncV1(EmpAssessmentModel empAssessmentModel)
        {
            try
            {
        ...
        /// <param name="empAssessmentModel"></param>
        /// <returns></returns>
        public async Task<JsonData> AddAssessmentAsyncV2(EmpAssessmentModel empAssessmentModel)
        {
            try
            {
                bool canAdd = false;
                ...
                if (Convert.ToInt32(_iClaims["RoleId"].Value) == Enums.DimensionElements.CompanyAdmin.GetHashCode() || canAdd)
                {
                    ...
                    var resultAssessmentModel = await _assessmentContext.AddAssessmentMainV2(empAssessmentModel);
                    return await GetAssessmentScoreById(resultAssessmentModel.assessmentId, resultAssessmentModel.Result);
                }
                else
                {
        ...
        /// <param name="empAssessmentModel"></param>
        /// <returns></returns>
        public async Task<JsonData> AddAssessmentAsyncV2_1(EmpAssessmentModel empAssessmentModel)
        {
            try
            {
                if (_permission.CheckActionPermission(_permission.GetPermissionParameters(Enums.Actions.TriggerEmployee, Enums.PermissionType.CanAdd)))
                {
                    ...
                    var resultAssessment = await _assessmentContext.AddAssessmentMainV2(empAssessmentModel);
                    return await GetAssessmentScoreById(resultAssessment.assessmentId, resultAssessment.Result);
                }
                else
                {
                    return JsonSettings.UserDataWithStatusMessage(null, Enums.StatusCodes.status_403.GetHashCode(), Messages.employeeAccessDenied);
        ...
        /// <param name="documentDetails"></param>
        /// <returns></returns>
        public async Task<JsonData> DeleteAssessmentAttachmentV2(EmpAssessmentDet empAssessmentDet)
        {
            try
            {
        ...
        /// <param name="empAssessmentDet"></param>
        /// <returns></returns>
        public async Task<JsonData> UpdateAssessmentCommentV2(EmpAssessmentDet empAssessmentDet)
        {
            try
            {
        ...
        /// <param name="remarkid"></param>
        /// <returns></returns>
        public async Task<JsonData> DeleteAssessmentCommentV2(int assessmentId, int remarkId, int userId)
        {
            try
            {
                if (_permission.CheckActionPermission(_permission.GetPermissionParameters(Enums.Actions.Comment, Enums.PermissionType.CanDelete)))
